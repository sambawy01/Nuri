...
// ─── LearningFlags ────────────────────────────────────────────────────────────
// Gap A: Display learning need flags (dyslexia, ADHD, dyscalculia) detected by analyzePatterns

function LearningFlags({ flags }) {
  if (!flags || flags.length === 0) return null;

  const labels = {
    dyslexia: { title: 'Dyslexia Pattern Detected', color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200' },
    adhd: { title: 'ADHD Pattern Detected', color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' },
    dyscalculia: { title: 'Dyscalculia Pattern Detected', color: 'text-teal-600', bg: 'bg-teal-50', border: 'border-teal-200' },
    autism: { title: 'Autism Pattern Detected', color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-200' },
  };

  return (
    <Card delay={0.26}>
      <SectionTitle icon={Lightbulb} title="Learning Insights" color="text-rose-500" />
      <div className="space-y-3">
        {flags.map((flag, i) => {
          const cfg = labels[flag.condition] || labels.dyslexia;
          return (
            <motion.div
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <h4 className={`font-extrabold text-sm ${cfg.color}`}>{cfg.title}</h4>
              </div>
              <p className="text-xs font-semibold text-gray-600 mb-1">Confidence: {flag.confidence}</p>
              <p className="text-xs text-gray-500 leading-relaxed">{flag.evidence}</p>
              <div className="mt-2 px-3 py-2 bg-white/60 rounded-lg text-xs text-gray-600 font-medium">
                {flag.suggestion}
              </div>
            </motion.div>
          );
      ...
      <p className="text-xs text-gray-400 mt-3 font-medium">
        These are automatically detected patterns, not diagnoses. Share with a specialist if concerned.
      </p>
    </Card>
  );
}
...
export default function ParentDashboardPage() {
  ...
  const { profile, reports, summary, predictions, badges, weeklyXP, weeklyTime, mistakePatterns, topicMastery, presence, learningFlags } = data;
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-16">
      ...
      <div className="px-4 -mt-10 space-y-4 max-w-2xl mx-auto">
        ...
        <LearningFlags flags={learningFlags} />
