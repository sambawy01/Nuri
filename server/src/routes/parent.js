...
router.get('/dashboard/:profileId', async (req, res, next) => {
  try {
    ...
    const [profile, reports, summary, predictions, badges, weeklyXP, weeklyTime, mistakePatterns, topicMastery, presenceSummary, learningFlags] = await Promise.all([
      ...
      pool.query(`SELECT condition, confidence, evidence, suggestion, created_at FROM learning_need_flags WHERE profile_id = $1 ORDER BY created_at DESC`, [profileId]).then(r => r.rows).catch(() => []),
    ]);
