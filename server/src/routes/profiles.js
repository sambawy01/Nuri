...
// POST /api/profiles/voice-login — voice-authenticated login
const speakerRecognition = require('../services/speaker-recognition');

router.post('/voice-login', async (req, res, next) => {
  try {
    ...
    // Retrieve full profile (same shape as /login, without PIN)
    const profileResult = await pool.query(
      'SELECT id, name, year_group, avatar_color, total_xp, current_level, streak_days, last_active_date FROM profiles WHERE id = $1',
      [match.profileId]
    );
    ...
    res.json({
      ...
      data: {
        ...profile,
        voiceConfidence: match.confidence,
      },
    });
  } catch (err) {
    next(err);
