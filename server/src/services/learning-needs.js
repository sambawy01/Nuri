...
async function analyzePatterns(profileId) {
  ...
  // ── Gap B: Persist detected flags to DB + auto-referral check ──
  // Save newly detected flags before running threshold checks
  if (flags.length > 0) {
    const insertPromises = flags.map(f =>
      pool.query(
  ...
  // Now check total flags including the ones just saved
  const totalFlags = await pool.query(
    `SELECT COUNT(*) as c FROM learning_need_flags WHERE profile_id = $1 AND created_at >= NOW() - INTERVAL '60 days'`,
    [profileId]
  );
  if (parseInt(totalFlags.rows[0].c) >= 2) {
    // Multiple distinct conditions flagged → suggest specialist assessment
    const alreadyReferred = await pool.query(
      `SELECT id FROM specialist_reports WHERE profile_id = $1 AND created_at >= NOW() - INTERVAL '90 days'`,
      [profileId]
    );
    if (alreadyReferred.rows.length === 0) {
      await pool.query(
        `INSERT INTO specialist_reports (profile_id, report_data)
         VALUES ($1, $2)`,
        [profileId, JSON.stringify({
          reason: 'multiple_learning_need_flags',
          triggered_by: flags.map(f => f.condition),
          total_flags_60d: parseInt(totalFlags.rows[0].c),
          recommended_specialist: 'educational_psychologist',
          urgency: 'medium',
